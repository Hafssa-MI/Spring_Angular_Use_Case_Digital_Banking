package net.hafssa.ebankbackend.rag;
...
public class DocumentIndexor {
    ...
    private @Qualifier("ollamaEmbeddingModel")EmbeddingModel embeddingModel;
...
//    @Bean
//   public SimpleVectorStore getVectorStore(@Qualifier("ollamaEmbeddingModel")EmbeddingModel embeddingModel){
...
//
//    }
    public SimpleVectorStore loadFile(MultipartFile pdfFile){
        SimpleVectorStore vectorStore =
                SimpleVectorStore.builder(embeddingModel).build();
        Path path = Path.of("src","main","resources","store");
        File file = new File(path.toFile(),fileStore);
